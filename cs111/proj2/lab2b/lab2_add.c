...
void add_lock(long long *pointer, long long value) {
	...
	// if (clock_gettime(CLOCK_MONOTONIC, &mend)) {   // check to see if there was an error 
	// 	fprintf(stderr, "Error with clock_gettime()\n");
	// 	exit(1);
	// }
	// blockedtime += (mend.tv_sec - mstart.tv_sec) * 1000000000 + (mend.tv_nsec - mstart.tv_nsec);
    long long sum = *pointer + value;
    if (opt_yield)
        sched_yield();
    *pointer = sum;
...
int main(int argc, char** argv)
{
	...
	printf(",%d,%d,%d,%lld,%lld,%llu,%lld\n", numthreads, numiterations, ops,totalTime, totalTime/ops, counter, blockedtime/ops);
	if(counter)
		exit(1);
	exit(0);
}