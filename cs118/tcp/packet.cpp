...
TCP_Packet::TCP_Packet(uint16_t seqnum, uint16_t acknum, uint16_t win, bool ack, bool syn,
    bool fin, uint8_t* data, ssize_t data_size){
	// Set Header Fields
	m_header.fields[SEQ] = seqnum;
	m_header.fields[ACK] = acknum;
	m_header.fields[WIN] = win;

    m_header.fields[FLAGS] |= fin | (syn << 1) | (ack << 2);
    ...
    // cout << "Creating packet with " << seqnum << " " << acknum;
}
...
//     m_byte_packet = other.m_byte_packet;
...
uint8_t* TCP_Packet::encode(){
    ...
    m_encoded_packet = new uint8_t[total_size];
...
// uint8_t* TCP_Packet::getPacketBytes(){

//     return m_byte_packet;
// }
...
bool TCP_Packet::setData(char* data, int data_size){
    if(data){
        ...
        for(ssize_t i = 0; i < data_size; i++){
            ...
            // ssize_t packet_size = m_data.size() + HEADER_SIZE;

        // uint8_t* m_byte_packet = new uint8_t[packet_size];
        // for(ssize_t i = HEADER_SIZE; i < packet_size; i++){
        //     m_byte_packet[i] = m_data[i-HEADER_SIZE];
        // }
        return true;
    }
    return false;
